using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata;

#nullable disable

namespace UploadAllFile.Models
{
    public partial class UploadFileContext : DbContext
    {
        public UploadFileContext()
        {
        }

        public UploadFileContext(DbContextOptions<UploadFileContext> options)
            : base(options)
        {
        }

       
        public virtual DbSet<AllDocument> AllDocuments { get; set; }


        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            if (!optionsBuilder.IsConfigured)
            {
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
                optionsBuilder.UseSqlServer("Server=DESKTOP-QTL6Q4F\\SQLEXPRESS;Database=UploadFile;Trusted_Connection=True;");
            }
        }

        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.HasAnnotation("Relational:Collation", "SQL_Latin1_General_CP1_CI_AS");

            modelBuilder.Entity<AllDocument>(entity =>
            {
                entity.ToTable("AllDocuments");

                entity.Property(e => e.Id).ValueGeneratedNever();

                entity.Property(e => e.DocumentDesciption).IsRequired();

                entity.Property(e => e.DocumentTitle).HasColumnName("DocumentTitle");

                entity.Property(e => e.DocumentURL).IsRequired();
                entity.Property(e => e.CreatationDate).IsRequired();

            });

            OnModelCreatingPartial(modelBuilder);
        }

        partial void OnModelCreatingPartial(ModelBuilder modelBuilder);
    }
}
